...
typedef struct {
  ...
  /*!< User callback called after get coordinates from touch controller for
   * apply user adjusting */
  void (*process_coordinates)(esp_lcd_touch_handle_t tp, uint16_t *x,
...
struct esp_lcd_touch_s {
   ...
   * @param point_num: Count of points touched (equals with count of items in x
   * and y array)
   * @param max_point_num: Maximum count of touched points to return (equals
   * with max size of x and y array)
   *
   * @return
   *      - Returns true, when touched and coordinates readed. Otherwise returns
   * false.
   */
   ...
   * false.
   */
  esp_err_t (*get_button_state)(esp_lcd_touch_handle_t tp, uint8_t n,
                                uint8_t *state);
#endif
 ...
 *     - ESP_FAIL               sending command error, slave hasn't ACK the
 * transfer
 ...
 * @param point_num: Count of points touched (equals with count of items in x
 * and y array)
 * @param max_point_num: Maximum count of touched points to return (equals with
 * max size of x and y array)
 *
 * @return
 *      - Returns true, when touched and coordinates readed. Otherwise returns
 * false.
 ...
 * @return
 *      - ESP_OK                    on success
 *      - ESP_ERR_NOT_SUPPORTED     if this function is not supported by
 * controller
 *      - ESP_ERR_INVALID_ARG       if bad button index
 */
esp_err_t esp_lcd_touch_get_button_state(esp_lcd_touch_handle_t tp, uint8_t n,
                                         uint8_t *state);
#endif
...
esp_err_t esp_lcd_touch_register_interrupt_callback(
    esp_lcd_touch_handle_t tp, esp_lcd_touch_interrupt_callback_t callback);
